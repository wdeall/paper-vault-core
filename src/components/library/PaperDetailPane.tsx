...
export function PaperDetailPane({ paperId }: { paperId: string }) {
  ...
  async function handleDelete(mode: "entry" | "entry+pdf" | "entry+pdf+note") {
    if (!detail) return;
    if (!confirmDelete) {
    ...
    try {
      await removePaper(detail.id, mode);
        ...
        <TabsContent value="danger" className="flex-1 overflow-y-auto p-3">
          <Card className="border-destructive/40 p-3">
            ...
            <p className="mb-2 text-xs text-muted-foreground">
              默认只删除数据库条目，PDF 和 Markdown 笔记作为用户资产保留。
            </p>
            <div className="space-y-2">
              ...
              >
                {confirmDelete ? "再次点击确认" : "仅删除条目"}
              </Button>
              <Button
                variant="outline"
                size="sm"
                className="w-full"
                onClick={() => handleDelete("entry+pdf")}
              >
                删除条目 + PDF
              </Button>
              <Button
                ...
                size="sm"
                className="w-full"
                onClick={() => handleDelete("entry+pdf+note")}
              >
                删除条目 + PDF + 笔记
              </Button>
            </div>
            <p className="mt-2 text-[10px] text-muted-foreground">
              建议先确认"仅删除条目"，需要时再手动清理文件。
            </p>
          </Card>
