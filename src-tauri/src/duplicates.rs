...
pub fn detect(
    ...
    if candidates.is_empty() {
        if let (Some(authors), Some(year)) = (authors, year) {
            ...
            if !last.is_empty() {
                let mut stmt = conn.prepare(
                    "SELECT id, title, authors, year FROM papers WHERE year = ?1 OR year = ?2",
                )?;
                ...
                for row in rows {
                    let (id, db_title, db_authors_json, db_year) = row?;
                    let db_authors: Vec<String> =
                        serde_json::from_str(&db_authors_json).unwrap_or_default();
                    if first_author_lastname(&db_authors) == last && db_year == Some(year) {
                        candidates.push(DuplicateCandidate {
                            paper_id: id,
                            title: db_title,
...
mod tests {
    ...
    fn detect_by_doi() {
        ...
        conn.execute(
            "INSERT INTO papers (id, title, doi, year, created_at, updated_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            params!["p1", "Existing", "10.1109/foo", 2024, now, now],
        ).unwrap();
